...
{
	//第一层优化:三数取中间大小的，交换到边上做key，
	//解决最坏的O(N*N)的情况
	//实现略

	//第二层优化：小区间优化
	//二叉树的角度：最下面一层的递归次数太多（占一半）
	//分割到小区间时，用直接插入排序
	//实现略
	int left = begin;
	int right = end;
...
void multiQuickSort(int* a, int begin, int end)//快排--大量重复数据时
{
	...
	multiQuickSort(a, begin, left - 1);
	multiQuickSort(a, right + 1, end);
}

void QuickSortNonR(int* a, int begin, int end)// 快速排序 非递归实现
{
	Stack st;
	StackInit(&st);
	StackPush(&st, begin);
...
//消耗了malloc空间+递归调用空间(看的深度，因为会利用用过的空间)
...
void CountSort(int* a, int n)
{
	...
	//排序
	int k = 0;
	for (int i = 0; i < range; i++)//注意是n还是range
	{
		while (countA[i]--)//计数了多少次，就排几次
		{
			a[k++] = i + min;
		}
	}
